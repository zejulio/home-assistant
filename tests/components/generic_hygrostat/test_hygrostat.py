...
ENTITY = "humidity.test"
ENT_SENSOR = "sensor.test"
ENT_SWITCH = "switch.test"
HUMIDIFY_ENTITY = "humidity.test_heat"
DRY_ENTITY = "humidity.test_cool"
...
async def test_setup_missing_conf(hass):
    ...
    with assert_setup_component(0):
        await async_setup_component(hass, "humidity", {"humidity": config})
...
async def test_humidifier_input_boolean(hass, setup_comp_1):
    ...
    assert await async_setup_component(
        ...
        DOMAIN,
        {
            "humidity": {
                ...
                "target_sensor": ENT_SENSOR,
                "initial_humidifier_mode": HUMIDIFIER_MODE_HUMIDIFY,
            }
        },
    )
    ...
    await hass.services.async_call(DOMAIN, SERVICE_SET_HUMIDITY, {ATTR_HUMIDITY: 32}, blocking=True)
...
async def test_setup_defaults_to_unknown(hass):
    ...
    assert HUMIDIFIER_MODE_OFF == hass.states.get(ENTITY).state
...
async def test_get_humidifier_modes(hass, setup_comp_2):
    """Test that the operation list returns the correct modes."""
    state = hass.states.get(ENTITY)
    modes = state.attributes.get("humidifier_modes")
    assert [HUMIDIFIER_MODE_HUMIDIFY, HUMIDIFIER_MODE_OFF] == modes


async def test_set_target_humidity(hass, setup_comp_2):
    """Test the setting of the target humidity."""
    await hass.services.async_call(DOMAIN, SERVICE_SET_HUMIDITY, {ATTR_HUMIDITY: 40}, blocking=True)
    state = hass.states.get(ENTITY)
    assert 40 == state.attributes.get("humidity")
    with pytest.raises(vol.Invalid):
        await hass.services.async_call(DOMAIN, SERVICE_SET_HUMIDITY, {ATTR_HUMIDITY: None}, blocking=True)
    state = hass.states.get(ENTITY)
    assert 40 == state.attributes.get("humidity")
...
async def test_set_away_mode(hass, setup_comp_2):
    ...
    await hass.services.async_call(DOMAIN, SERVICE_SET_HUMIDITY, {ATTR_HUMIDITY: 45}, blocking=True)
    await hass.services.async_call(DOMAIN, SERVICE_SET_PRESET_MODE, {ATTR_PRESET_MODE: PRESET_AWAY}, blocking=True)
    state = hass.states.get(ENTITY)
...
async def test_set_away_mode_and_restore_prev_humidity(hass, setup_comp_2):
    ...
    state = hass.states.get(ENTITY)
    assert 35 == state.attributes.get("humidity")
    await hass.services.async_call(DOMAIN, SERVICE_SET_PRESET_MODE, {ATTR_PRESET_MODE: PRESET_NONE}, blocking=True)
    state = hass.states.get(ENTITY)
...
async def test_set_target_humidity_humidifier_off(hass, setup_comp_2):
    ...
    await hass.async_block_till_done()
    await hass.services.async_call(DOMAIN, SERVICE_SET_HUMIDITY, {ATTR_HUMIDITY: 36}, blocking=True)
    assert 2 == len(calls)
    call = calls[0]
    assert HASS_DOMAIN == call.domain
...
async def test_running_when_humidifier_mode_is_off(hass, setup_comp_2):
    """Test that the switch turns off when enabled is set False."""
    calls = _setup_switch(hass, True)
    await hass.services.async_call(DOMAIN, SERVICE_SET_HUMIDITY, {ATTR_HUMIDITY: 45}, blocking=True)
    await hass.services.async_call(DOMAIN, SERVICE_SET_HUMIDIFIER_MODE, {ATTR_HUMIDIFIER_MODE: HUMIDIFIER_MODE_OFF}, blocking=True)
    assert 1 == len(calls)
    call = calls[0]
    assert HASS_DOMAIN == call.domain
    assert SERVICE_TURN_OFF == call.service
...
async def test_no_state_change_when_humidifier_mode_off(hass, setup_comp_2):
...
async def test_humidifier_mode_heat(hass, setup_comp_2):
    ...
    await hass.async_block_till_done()
    calls = _setup_switch(hass, False)
    await hass.services.async_call(DOMAIN, SERVICE_SET_HUMIDIFIER_MODE, {ATTR_HUMIDIFIER_MODE: HUMIDIFIER_MODE_HUMIDIFY}, blocking=True)
    assert 1 == len(calls)
    call = calls[0]
    assert HASS_DOMAIN == call.domain
    assert SERVICE_TURN_ON == call.service
        ...
        async_setup_component(
            ...
            DOMAIN,
            {
                "humidity": {
                    ...
                    "target_sensor": ENT_SENSOR,
                    "dry_mode": True,
                    "initial_humidifier_mode": HUMIDIFIER_MODE_DRY,
                }
            },
        )
    )
...
async def test_turn_away_mode_on_cooling(hass, setup_comp_3):
    ...
    _setup_sensor(hass, 40)
    await hass.async_block_till_done()
    await hass.services.async_call(DOMAIN, SERVICE_SET_HUMIDITY, {ATTR_HUMIDITY: 34}, blocking=True)
    await hass.services.async_call(DOMAIN, SERVICE_SET_PRESET_MODE, {ATTR_PRESET_MODE: PRESET_AWAY}, blocking=True)
    state = hass.states.get(ENTITY)
    assert 30 == state.attributes.get("humidity")


async def test_humidifier_mode_cool(hass, setup_comp_3):
    ...
    await hass.async_block_till_done()
    calls = _setup_switch(hass, False)
    await hass.services.async_call(DOMAIN, SERVICE_SET_HUMIDIFIER_MODE, {ATTR_HUMIDIFIER_MODE: HUMIDIFIER_MODE_DRY}, blocking=True)
    assert 1 == len(calls)
    call = calls[0]
    assert HASS_DOMAIN == call.domain
    assert SERVICE_TURN_ON == call.service
...
async def test_restore_state(hass):
    ...
    state = hass.states.get("humidity.test_hygrostat")
...
async def test_restore_state_uncoherence_case(hass):
    ...
    calls = _setup_switch(hass, False)
    _setup_sensor(hass, 35)
    await _setup_humidity(hass)
    await hass.async_block_till_done()

    state = hass.states.get(ENTITY)
    assert 40 == state.attributes[ATTR_HUMIDITY]
    assert HUMIDIFIER_MODE_OFF == state.state
    assert 0 == len(calls)
...
async def _setup_humidity(hass):
    assert await async_setup_component(
        hass,
        DOMAIN,
        {
...
def _mock_restore_cache(hass, humidity=40, humidifier_mode=HUMIDIFIER_MODE_OFF):
    mock_restore_cache(
        hass,
        (
            State(
