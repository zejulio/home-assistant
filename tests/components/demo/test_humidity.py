...
def test_setup_params(hass):
    ...
    assert state.attributes.get(ATTR_HUMIDIFIER_MODES) == [
        "dry",
        "off",
    ]
...
async def test_set_humidifier(hass):
    ...
    await hass.services.async_call(
        DOMAIN,
        SERVICE_SET_HUMIDIFIER_MODE,
        {ATTR_HUMIDIFIER_MODE: HUMIDIFIER_MODE_HUMIDIFY, ATTR_ENTITY_ID: ENTITY_DEHUMIDIFIER},
        blocking=True,
    )
    await hass.async_block_till_done()
...
async def test_turn_on(hass):
    ...
    await hass.services.async_call(
        DOMAIN,
        SERVICE_SET_HUMIDIFIER_MODE,
        {ATTR_HUMIDIFIER_MODE: HUMIDIFIER_MODE_OFF, ATTR_ENTITY_ID: ENTITY_DEHUMIDIFIER},
        blocking=True,
    )
    state = hass.states.get(ENTITY_DEHUMIDIFIER)
