...
@pytest.mark.parametrize("state", [HUMIDIFIER_MODE_AUTO, HUMIDIFIER_MODE_HUMIDIFY, HUMIDIFIER_MODE_OFF])
...
async def test_multiple_state(hass):
    ...
    await async_reproduce_states(
        hass, [State(ENTITY_1, HUMIDIFIER_MODE_HUMIDIFY), State(ENTITY_2, HUMIDIFIER_MODE_AUTO)]
    )
    ...
    assert any(
        call.data == {"entity_id": ENTITY_1, "humidifier_mode": HUMIDIFIER_MODE_HUMIDIFY}
        for call in calls_1
    )
    assert any(
...
async def test_state_with_context(hass):
    ...
    await async_reproduce_states(hass, [State(ENTITY_1, HUMIDIFIER_MODE_HUMIDIFY)], context)

    await hass.async_block_till_done()

    assert len(calls) == 1
    assert calls[0].data == {"entity_id": ENTITY_1, "humidifier_mode": HUMIDIFIER_MODE_HUMIDIFY}
