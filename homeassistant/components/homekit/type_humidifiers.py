...
HC_HASS_TO_HOMEKIT = {
    OPERATION_MODE_HUMIDIFY_DRY: 0,
    OPERATION_MODE_HUMIDIFY: 1,
    OPERATION_MODE_DRY: 2,
}
...
class HumidifierDehumidifier(HomeAccessory):
    ...
    def __init__(self, *args):
        ...
        if features & SUPPORT_WATER_LEVEL:
            self.chars.append(CHAR_WATER_LEVEL)

        target_humidity_char = CHAR_HUMIDIFIER_THRESHOLD_HUMIDITY
        modes = self.hass.states.get(self.entity_id).attributes.get(
            ATTR_OPERATION_MODES
        )
        if (
            modes
            and OPERATION_MODE_DRY in modes
            and OPERATION_MODE_HUMIDIFY not in modes
        ):
        ...
        self.char_target_humidifier_dehumidifier = serv_humidifier_dehumidifier.configure_char(
            CHAR_TARGET_HUMIDIFIER_DEHUMIDIFIER,
            value=0,
            setter_callback=self.set_humidifier_dehumidifier,
        )
        ...
        # If the device supports it: water level characteristics
        self.char_water_level = None
        if CHAR_WATER_LEVEL in self.chars:
            self.char_water_level = serv_humidifier_dehumidifier.configure_char(
                CHAR_WATER_LEVEL, value=100.0
            )
    ...
    def set_humidifier_dehumidifier(self, value):
        ...
        self.call_service(DOMAIN, SERVICE_SET_OPERATION_MODE, params, hass_value)
    ...
    def set_target_humidity(self, humidity):
        ...
        self.call_service(DOMAIN, SERVICE_SET_HUMIDITY, params, f"{humidity}%")
    ...
    def update_state(self, new_state):
        ...
        if operation_mode:
            if not self._flag_humidifier_dehumidifier:
                if operation_mode != OPERATION_MODE_OFF:
                    self._last_active_mode = (
                        ...
                        if operation_mode in HC_HASS_TO_HOMEKIT
            ...
            if not self._flag_active:
                self.char_active.set_value(operation_mode != OPERATION_MODE_OFF)
        self._flag_humidifier_dehumidifier = False
        self._flag_active = False

        # Update water level if characteristic exists
        if self.char_water_level:
            water_level = new_state.attributes.get(ATTR_WATER_LEVEL)
            if isinstance(water_level, (int, float)):
                self.char_water_level.set_value(water_level)

        # Set current operation mode for supported hygrostats
        if operation_mode and operation_mode == OPERATION_MODE_OFF:
            humidifier_action = CURRENT_HUMIDIFIER_OFF
        else:
            humidifier_action = new_state.attributes.get(ATTR_HUMIDIFIER_ACTION)
